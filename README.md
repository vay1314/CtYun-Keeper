...
4. 分别设置登录、时长和 AI 对话三个每日积分任务的 Cron 计划与各自的 0–120 分钟最大随机延迟。
5. 时长任务在关闭保活或定时窗口之外运行时，会临时连接云电脑，累计满 1 小时后自动断开。
6. 在“任务中心”可以手动执行任务、查询当天任务状态，并查看最近 3 个月的积分收入、消耗和过期明细。
7. 账号列表可直接启停账号或单项定时任务；停用账号会终止该账号正在运行的任务和保活连接。
8. 需要兑换时，在账号列表进入“兑换”，从实时商品和云电脑列表选择目标并主动启用；自动兑换可单独配置 0–120 分钟的最大随机延迟。
